...
# Import the OpenCacheViewer command
from .open_cache_viewer import OpenCacheViewer

# Register the OpenCacheViewer command
...
class _CacheViewer(CommandManager):
    """The FEM_CacheViewer command definition"""

    def __init__(self):
        super().__init__()
        self.pixmap = "document-open"
        self.menutext = Qt.QT_TRANSLATE_NOOP("FEM_CacheViewer", "Open Cache Viewer")
        self.tooltip = Qt.QT_TRANSLATE_NOOP(
            "FEM_CacheViewer", "Opens the LS-DYNA keyword cache viewer"
        )
        self.is_active = "always"

    def Activated(self):
        OpenCacheViewer().Activated()
...
class _MaterialEditor(CommandManager):
    ...
    def Activated(self):
        FreeCADGui.runCommand("Material_Editor")
...
class _OpenRadiossContextManager:
    ...
    def __enter__(self):
        FreeCAD.ActiveDocument.openTransaction("Create SolverOpenRadioss")
        FreeCADGui.addModule("ObjectsFem")
        FreeCADGui.addModule("FemGui")
        FreeCADGui.doCommand(
            f"{self.cli_name} = ObjectsFem.{self.make_name}(FreeCAD.ActiveDocument)"
        )
        # Load executable path from preferences
        openradioss_prefs = FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/Fem/OpenRadioss")
        executable_path = openradioss_prefs.GetString("ExecutablePath", "/home/nemo/Dokumente/Software/OpenRadioss_linux64/OpenRadioss/exec/engine_linux64_gf")
        # Set the executable to OpenRadioss path
        FreeCADGui.doCommand(f"{self.cli_name}.Executable = '{executable_path}'")
...
class _SolverOpenRadioss(CommandManager):
    "The FEM_SolverOpenRadioss command definition"

    def __init__(self):
        super().__init__()
        self.pixmap = "FEM_SolverStandard"  # Use standard FEM solver icon
        self.menutext = Qt.QT_TRANSLATE_NOOP(
            "FEM_SolverOpenRadioss", "Solver OpenRadioss"
        )
        self.accel = "S, O"
        self.tooltip = Qt.QT_TRANSLATE_NOOP(
            "FEM_SolverOpenRadioss",
            "Creates a FEM solver using OpenRadioss",
        )
        self.is_active = "with_analysis"
        self.do_activated = "add_obj_on_gui_expand_noset_edit"

    def Activated(self):
        with _OpenRadiossContextManager("makeSolverOpenRadioss", "solver") as cm:
            has_nonlinear_material_obj = False
            for m in self.active_analysis.Group:
                if is_of_type(m, "Fem::MaterialMechanicalNonlinear"):
                    has_nonlinear_material_obj = True
...
class _OpenRadiossGui(CommandManager):
    "The FEM_OpenRadiossGui command definition"

    def __init__(self):
        super().__init__()
        self.pixmap = "FEM_SolverRun"  # Use standard FEM run icon
        self.menutext = Qt.QT_TRANSLATE_NOOP("FEM_OpenRadiossGui", "OpenRadioss GUI")
        self.tooltip = Qt.QT_TRANSLATE_NOOP(
            "FEM_OpenRadiossGui", "Launches the OpenRadioss GUI for simulation setup"
        )
        self.is_active = "always"
    ...
    def Activated(self):
        ...
        gui_script = "/home/nemo/Dokumente/Software/OpenRadioss_linux64/OpenRadioss/openradioss_gui/OpenRadioss_gui.py"
        if not os.path.exists(gui_script):
            FreeCAD.Console.PrintError(f"OpenRadioss GUI script not found at {gui_script}\n")
            return
        ...
        # Check if there is an active analysis
        analysis = FemGui.getActiveAnalysis()
        if not analysis:
            FreeCAD.Console.PrintError("No active FEM analysis found. Please create or activate an analysis.\n")
            return

        # For now, just launch the GUI; user can load the file manually
        try:
            subprocess.Popen(["python3", gui_script], cwd=os.path.dirname(gui_script))
        except Exception as e:
            FreeCAD.Console.PrintError(f"Error launching GUI: {e}\n")


class _UpdateKFile(CommandManager):
    "The FEM_UpdateKFile command definition"

    def __init__(self):
        super().__init__()
        self.pixmap = "FEM_SolverStandard"
        self.menutext = Qt.QT_TRANSLATE_NOOP("FEM_UpdateKFile", "Update .k File")
        self.tooltip = Qt.QT_TRANSLATE_NOOP(
            "FEM_UpdateKFile", "Exports the current FEM analysis to a .k file"
        )
        self.is_active = "with_analysis"
    ...
    def Activated(self):
        ...
        analysis = FemGui.getActiveAnalysis()
        if not analysis:
            FreeCAD.Console.PrintError("No active FEM analysis found.\n")
            return

        # Export to .k file
        try:
            import femsolver.calculix.solver as ccx_solver
            # Create a temporary solver to write the input
            temp_solver = ccx_solver.create(FreeCAD.ActiveDocument, "TempSolver")
            temp_solver.AnalysisType = 0  # Static
            analysis.addObject(temp_solver)
            FreeCAD.ActiveDocument.recompute()

            # Write the input file
            k_file = os.path.join(os.path.dirname(FreeCAD.ActiveDocument.FileName) if FreeCAD.ActiveDocument.FileName else "/tmp", "fem_export.k")
            temp_solver.write_input(k_file)

            # Remove the temp solver
            FreeCAD.ActiveDocument.removeObject(temp_solver.Name)

            FreeCAD.Console.PrintMessage(f".k file exported to: {k_file}\n")
        except Exception as e:
            FreeCAD.Console.PrintError(f"Error exporting .k file: {e}\n")
...
class _OpenRadiossGui(CommandManager):
    ...
    def __init__(self):
        ...
        self.menutext = Qt.QT_TRANSLATE_NOOP("FEM_OpenRadiossGui", "OpenRadioss GUI")
        self.tooltip = Qt.QT_TRANSLATE_NOOP(
            "FEM_OpenRadiossGui", "Opens the OpenRadioss graphical user interface"
        )
        self.is_active = "always"
    ...
    def Activated(self):
        ...
        try:
            ...
            # Find the OpenRadioss GUI script
            gui_script = os.path.join(os.path.dirname(__file__), '..', 'gui', 'openradioss_gui.py')
            if os.path.exists(gui_script):
                # For now, just show a message since GUI may need more setup
                FreeCAD.Console.PrintMessage("OpenRadioss GUI would be launched here\n")
            else:
                FreeCAD.Console.PrintError("OpenRadioss GUI script not found\n")

        except Exception as e:
            FreeCAD.Console.PrintError(f"Error launching OpenRadioss GUI: {e}\n")
...
class _SolverOpenRadioss(CommandManager):
    ...
    def __init__(self):
        super().__init__()
        self.menutext = Qt.QT_TRANSLATE_NOOP("FEM_SolverOpenRadioss", "OpenRadioss Solver")
        self.tooltip = Qt.QT_TRANSLATE_NOOP(
            "FEM_SolverOpenRadioss", "Creates a FEM solver for OpenRadioss"
        )
        self.is_active = "with_analysis"
        self.do_activated = "add_obj_on_gui_expand_noset_edit"

    def Activated(self):
        # Create an OpenRadioss solver
        FreeCAD.ActiveDocument.openTransaction("Create OpenRadioss Solver")
        FreeCADGui.addModule("ObjectsFem")

        # Create the solver object
        solver_name = "OpenRadiossSolver"
        FreeCADGui.doCommand(
            f"ObjectsFem.makeSolverOpenRadioss(FreeCAD.ActiveDocument, '{solver_name}')"
        )

        # Add to active analysis if available
        import FemGui
        analysis = FemGui.getActiveAnalysis()
        if analysis:
            FreeCADGui.doCommand(
...
class _FemKeywordEditor(CommandManager):
    "The FEM_FemKeywordEditor command definition"

    def __init__(self):
        super().__init__()
        self.pixmap = "FEM_SolverStandard"
        self.menutext = Qt.QT_TRANSLATE_NOOP("FEM_FemKeywordEditor", "FEM Keyword Editor")
        self.tooltip = Qt.QT_TRANSLATE_NOOP(
            "FEM_FemKeywordEditor", "Opens the FEM keyword editor with dynamic CFG integration"
        )
        self.is_active = "with_solver"

    def Activated(self):
        # Open the FEM keyword editor dialog with dynamic CFG integration
        try:
            from ..gui.openradioss_keyword_editor import OpenRadiossKeywordEditorDialog
        except ImportError:
            # Fallback for direct execution or module issues
            import sys
            import os
            ...
            from openradioss_keyword_editor import OpenRadiossKeywordEditorDialog

        # Create and show the dialog
        dialog = OpenRadiossKeywordEditorDialog()
        dialog.exec_()
...
FreeCADGui.addCommand("FEM_SolverOpenRadioss", _SolverOpenRadioss())
...
FreeCADGui.addCommand("FEM_OpenRadiossGui", _OpenRadiossGui())
FreeCADGui.addCommand("FEM_UpdateKFile", _UpdateKFile())
FreeCADGui.addCommand("FEM_FemKeywordEditor", _FemKeywordEditor())
FreeCADGui.addCommand("FEM_SolverZ88", _SolverZ88())
# Freshly added Commands
