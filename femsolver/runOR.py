...
def run_fem_solver(solver, working_dir=None):
    ...
    if solver.Proxy.Type == "Fem::SolverOpenRadioss":
        from femtools.runORtools import FemToolsOR

        FreeCAD.Console.PrintMessage("Run of OpenRadioss solver started.\n")
        fea = FemToolsOR(solver=solver)
        ...
        if working_dir is None:
            ...
            if not message:
                ...
                FreeCAD.Console.PrintError(f"Houston, we have a problem...!\n{message}\n")
        FreeCAD.Console.PrintMessage("Run of OpenRadioss solver finished.\n")
    else:
