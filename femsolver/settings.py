...
def get_launch_paraview(name):
    """Check whether "launch_paraview" is set for solver.

    Returns ``True`` if the "launch_paraview" setting/parameter is set for the
    solver with the id *name*. Returns ``False`` otherwise. If the solver is
    not supported ``None`` is returned.
    ...
    """
    if name in _SOLVER_PARAM:
        return _SOLVER_PARAM[name].get_launch_paraview()
    else:
        FreeCAD.Console.PrintError(
            "Settings solver name: {} not found in "
            "solver settings modules _SOLVER_PARAM dirctionary.\n".format(name)
...
class _SolverDlg:
    ...
    PARAVIEW_LAUNCH_PARAM = "LaunchParaViewAfterAnalysis"
    ...
    def get_binary(self, silent=False):
        ...
        if os.path.isabs(binary):
            # For absolute paths, just check if the file exists
            if os.path.isfile(binary):
                the_found_binary = binary
                FreeCAD.Console.PrintLog(f"Found solver binary path (absolute): {the_found_binary}\n")
            else:
                the_found_binary = None
                if not silent:
                    FreeCAD.Console.PrintError(
                        f"The binary has not been found. Full binary search path: {binary}\n"
                    )
        else:
            # For relative paths or command names, use which to find in PATH
    ...
    def get_launch_paraview(self):
        return self.param_group.GetBool(self.PARAVIEW_LAUNCH_PARAM, False)
...
_SOLVER_PARAM = {
    ...
    "OpenRadioss": _SolverDlg(
        default="/home/nemo/Dokumente/Software/OpenRadioss_linux64/OpenRadioss/exec/engine_linux64_gf",
        param_path=_PARAM_PATH + "OpenRadioss",
        use_default="UseStandardOpenRadiossEngineLocation",
        custom_path="openRadiossEngineBinaryPath",
    ),
    "OpenRadiossStarter": _SolverDlg(
        default="/home/nemo/Dokumente/Software/OpenRadioss_linux64/OpenRadioss/exec/starter_linux64_gf",
        param_path=_PARAM_PATH + "OpenRadioss",
        use_default="UseStandardOpenRadiossStarterLocation",
        custom_path="openRadiossStarterBinaryPath",
    ),
    "OpenRadiossAnimToVTK": _SolverDlg(
        default="/home/nemo/Dokumente/Software/OpenRadioss_linux64/OpenRadioss/exec/anim_to_vtk",
        param_path=_PARAM_PATH + "OpenRadioss",
        use_default="UseStandardOpenRadiossAnimToVTKLocation",
        custom_path="openRadiossAnimToVTKBinaryPath",
    ),
}
