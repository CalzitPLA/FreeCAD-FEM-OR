...
def handle_fluidsection_liquid_inlet_outlet(inpfile, ORwriter):

    if not ORwriter.member.geos_fluidsection:
        return inpfile

    # Fluid sections:
    # fluidsection Liquid inlet outlet objs  requires special element definition
    # to fill ORwriter.FluidInletoutlet_ele list the mat_geo_sets are needed
    # thus this has to be after the creation of mat_geo_sets
    # different pipe cross sections will generate mat_geo_sets

    ORwriter.FluidInletoutlet_ele = []
    ORwriter.fluid_inout_nodes_file = join(
        ORwriter.dir_name, f"{ORwriter.mesh_name}_inout_nodes.txt"
    )
    ...
    for matgeoset in ORwriter.mat_geo_sets:
        ...
        for elid in matgeoset["ccx_elset"]:
            ...
            if (elsetchanged is False) and (fluidsec_obj.LiquidSectionType == "PIPE INLET"):
                ...
                ORwriter.FluidInletoutlet_ele.append(
    ...
    if is_fluidsection_inoutlet_setnames_possible(ORwriter.mat_geo_sets) is not None:
        ...
        meshtools.use_correct_fluidinout_ele_def(
            ORwriter.FluidInletoutlet_ele, ORwriter.femmesh_file, ORwriter.fluid_inout_nodes_file
        )
        inpfile = codecs.open(ORwriter.file_name, "a", encoding="utf-8")
...
def write_constraints_fluidsection(f, ORwriter):
    if not ORwriter.member.geos_fluidsection:
        return
    if ORwriter.analysis_type not in ["thermomech"]:
        return

    # write constraint to file
    f.write("\n***********************************************************\n")
    f.write("** FluidSection constraints\n")
    if os.path.exists(ORwriter.fluid_inout_nodes_file):
        inout_nodes_file = open(ORwriter.fluid_inout_nodes_file)
        lines = inout_nodes_file.readlines()
        inout_nodes_file.close()
    else:
        FreeCAD.Console.PrintError(
            f"1DFlow inout nodes file not found: {ORwriter.fluid_inout_nodes_file}\n"
        )
    # get nodes
    ORwriter.get_constraints_fluidsection_nodes()
    for femobj in ORwriter.member.geos_fluidsection:
