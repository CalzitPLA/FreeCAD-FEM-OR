...
def extract_nodesets(analysis, mesh_obj):
    """Extract temperature constraint nodesets from the analysis.
    ...
    # Find all temperature constraints in the analysis
    temp_constraints = [obj for obj in analysis.Group 
                       if hasattr(obj, 'TypeId') and 
                       obj.TypeId == 'Fem::ConstraintTemperature']
    
    for constraint in temp_constraints:
        nodeset_name = f"temp_{constraint.Name}"
...
def write_meshdata_constraint(f, femobj, temp_obj, ORwriter):
...
def write_constraint(f, femobj, temp_obj, ORwriter):
