...
def write_femelement_material(f, ORwriter):

    # floats read from ccx should use {:.13G}, see comment in writer module
    # see unit comment in writer module

    def is_density_needed():
        if ORwriter.analysis_type == "frequency":
            return True
        if (
            ORwriter.analysis_type == "thermomech"
            and not ORwriter.solver_obj.ThermoMechSteadyState
        ):
            return True
        if ORwriter.member.cons_centrif:
            return True
        if ORwriter.member.cons_selfweight:
            return True
        return False
    ...
    for femobj in ORwriter.member.mats_linear:
        ...
        if ORwriter.solver_obj.MaterialNonlinearity == "nonlinear":
            ...
            for nlfemobj in ORwriter.member.mats_nonlinear:
