...
def write_step_output(f, ORwriter):

    f.write("\n{}\n".format(59 * "*"))
    f.write("** Outputs --> frd file\n")
    if (
        ORwriter.member.geos_beamsection
        or ORwriter.member.geos_shellthickness
        or ORwriter.member.geos_fluidsection
    ):
        if ORwriter.solver_obj.BeamShellResultOutput3D is False:
    ...
    if not ORwriter.member.geos_fluidsection:
        ...
        if ORwriter.analysis_type == "thermomech":
            variables += ", HFL"
        if ORwriter.solver_obj.MaterialNonlinearity == "nonlinear":
            variables += ", PEEQ"
        ...
        if ORwriter.member.cons_fixed or ORwriter.member.cons_displacement:
            f.write("** outputs --> dat file\n")
        if ORwriter.member.cons_fixed:
            # reaction forces for all Constraint fixed
            f.write("** reaction forces for Constraint fixed\n")
            for femobj in ORwriter.member.cons_fixed:
                # femobj --> dict, FreeCAD document object is femobj["Object"]
                f.write("*NODE PRINT, NSET={}, TOTALS=ONLY\n".format(femobj["Object"].Name))
                f.write("RF\n")
        if ORwriter.member.cons_displacement:
            ...
            for femobj in ORwriter.member.cons_displacement:
                # femobj --> dict, FreeCAD document object is femobj["Object"]
                if (
        ...
        if ORwriter.member.cons_fixed or ORwriter.member.cons_displacement:
            f.write("\n")
        f.write(f"*OUTPUT, FREQUENCY={ORwriter.solver_obj.OutputFrequency}")
        ...
        # f.write("*NODE PRINT , NSET=" + ORwriter.ccx_nall + "\n")
        # f.write("U \n")
        # f.write("*EL PRINT , ELSET=" + ORwriter.ccx_eall + "\n")
        # f.write("S \n")
