...
def makeSolverOpenRadioss(doc, name="SolverOpenRadioss"):
    """makeSolverOpenRadioss(document, [name]):
    makes an OpenRadioss solver object"""
    obj = doc.addObject("Fem::FemSolverObjectPython", name)
    obj.addProperty("App::PropertyString", "Executable", "Base", "Path to the OpenRadioss executable")
    # Set Proxy for task dialog support - use the solver's own proxy class
    try:
        from femsolver.OpenRadioss.solver import Proxy
        if FreeCAD.GuiUp:
            from femviewprovider.view_solver_openradioss import VPSolverOpenRadioss
            obj.ViewObject.Proxy = VPSolverOpenRadioss(obj.ViewObject)
        else:
            obj.Proxy = Proxy(obj)
        # Make createMachine accessible on the object
        obj.createMachine = lambda *args, **kwargs: obj.ViewObject.Proxy.createMachine(obj, *args, **kwargs)
        print(f"DEBUG: Proxy and createMachine set successfully on {obj.Name}")
